...
<img src="assets/en-global/FRITZScope-Preview-Cover-4x5-EN.png" alt="FRITZ!Scope preview — work in progress" width="560">
...
O FRITZ!Scope procura tornar tudo isso mais direto: abrir o painel, perceber imediatamente o que está a acontecer e aprofundar apenas os detalhes necessários — sem configurações desnecessárias.

A preferência atual é disponibilizar a aplicação principal gratuitamente, com a possibilidade de apoio ou donativo único e totalmente opcional. Não está planeada qualquer subscrição recorrente para as funcionalidades essenciais. O modelo final ainda não está decidido e será confirmado com o contributo da comunidade antes de uma versão estável.
...
FRITZ!Scope aims to make that experience more direct: open the dashboard, understand what is happening immediately, and explore the details only when they are needed — without unnecessary configuration.

The current preference is to offer the core application free of charge, with completely optional one-time support or donations. No recurring subscription is planned for essential features. The final release model has not yet been decided and will be confirmed with community input before a stable release.
...
FRITZ!Scope soll diesen Ablauf vereinfachen: Dashboard öffnen, den aktuellen Zustand sofort verstehen und nur bei Bedarf weitere Details aufrufen — ohne unnötige Konfiguration.

Der derzeit bevorzugte Ansatz ist eine kostenlose Kernanwendung mit der Möglichkeit einer freiwilligen, einmaligen Unterstützung oder Spende. Für wesentliche Funktionen ist kein wiederkehrendes Abonnement geplant. Das endgültige Veröffentlichungsmodell steht noch nicht fest und soll vor einer stabilen Veröffentlichung unter Einbeziehung der Community bestätigt werden.
...
![FRITZ!Scope network diagnostics preview](assets/en-global/FRITZScope-Preview-Diagnostics-16x9-EN.png)

## Pré-visualização PT-PT

![Pré-visualização principal do FRITZ!Scope](assets/pt-pt/FRITZScope-Preview-Principal-16x9-PT.png)

![Pré-visualização do diagnóstico do FRITZ!Scope](assets/pt-pt/FRITZScope-Preview-Diagnostico-16x9-PT.png)
